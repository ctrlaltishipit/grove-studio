...
- 2026-08-22 (late) · SUBMISSION PUSHED: github.com/ctrlaltishipit/grove-app, 56 files, commit d6a2311. api/synthesise.py (missing from the delivered package) is in; no keys in the repo; LLM_MODEL default gemini-3.5-flash-lite. Remaining tonight: Supabase project (anon sign-ins ON; SQL order 01 -> 03 -> 02 -> 05, NOT 04) then Vercel import + 6 env vars.
- 2026-08-22 (late) · v1 ROUND 2 — every finding from the two adversarial reviews is closed. Security: `join_session()` SECURITY DEFINER RPC is now the ONLY write path to participants (direct INSERT revoked; UPDATE column-limited to display_name/last_seen_at), so the join code — not the session uuid — is the capability again; notes insert/update/delete now require the participant row to belong to notes.session_id (the old policies let a note be inserted or moved into any session whose uuid you held, inflating observer_count); notes UPDATE column-limited to (body, kind); get_roster/get_public_roster join notes on session too; new `rls_status()` and /api/health "rls" field so no URL is shared while RLS is off. Engine: re-synthesis of a synthesised session is creator-only (403 NOT_CREATOR); notes whose participant is outside the session are discarded before counting and the phantom-observer append loop is deleted.
- 2026-08-22 (late) · THE AUDIT NOW BITES. The first independence-audit.mjs reported "clean" on a tree that leaked notes three ways — the reviewer defeated it with backticked from(`notes`), PostgREST embedding participants->notes(body), ASI-merged statements, an api/dump.ts, a SECURITY DEFINER returning setof notes, a single-quoted function body, a multi-line alter publication, a view over notes, and disable row level security. Rules 1/2/4/5/8 hardened + new rule 11; `npm run audit:selftest` (scripts/independence-audit.selftest.mjs) seeds all 20 bypasses into throwaway repos and fails CI if any stops being caught. Its fake key is assembled at runtime rather than excluding the file from the secret scan — an excluded file is exactly where a real key would hide.
- 2026-08-22 (late) · Checks green on master: tsc, build, audit clean, audit self-test 21/21, 19 vitest, 53 pytest.
- 2026-08-22 (late) · **LIVE URL EXISTS — the submission gate is closed.** https://grove-app-psi.vercel.app (Vercel Hobby, GitHub ctrlaltishipit/grove-app) on Supabase project crxuledyodwbiemhxtbu (Mumbai). Verified on production, not locally: site 200; /api/synthesise returns JSON 405 (rewrite correct); deep links /s/<id>/findings and /join/GRVDEM survive refresh; bundle contains the URL + publishable key and NEITHER the service key nor the Google key; anonymous sign-in works; join + own-lane write + roster counts all work.
- 2026-08-22 (late) · RLS APPLIED AND ATTACKED. As a fresh anonymous visitor with the public key: read-all-notes → 0 rows; unfiltered body scan → 0 rows; participants→notes(body) embedding → 0 exposed. Own lane still readable, roster still correct. The independence receipt is now a fact about the database, not about the client code.
- 2026-08-22 (late) · SYNTHESIS VERIFIED ON PRODUCTION: 9.7 s, 3 observers, 8 findings — three at 3-of-3, one 2-of-3 carrying the planted cost-vs-wait-time disagreement, two single-observer greys. Model gemini-3.5-flash-lite. GRVDEM session status is now 'synthesised' with findings written.
- 2026-08-22 (late) · Supabase now issues NEW-FORMAT keys (sb_publishable_… / sb_secret_…), not the legacy eyJ JWTs. Both work with supabase-js and with the stdlib PostgREST calls in api/synthesise.py — tested. Any doc that says "starts with eyJ" is stale.
- 2026-08-22 (late) · TRAP HIT: toggling "Allow anonymous sign-ins" in Supabase does nothing until **Save changes** is clicked. It failed silently with anonymous_provider_disabled and would have died on stage. Always verify with a real POST /auth/v1/signup, never by looking at the toggle.
